			  ...
			  ) 
{
	...
	if (local_id < nclusters)
     ...
     const unsigned int point_size = get_global_size(0);
		for (point_id = get_global_id(0);point_id < npoints; point_id += point_size)//if (point_id < npoints)
		{
			float min_dist=FLT_MAX;
			float feature_tmp[NUM_FEATURE];
			//#pragma unroll UNROLL_FACTOR_READ
			//for (char jj = 0; jj < NUM_FEATURE; jj++)
			 // feature_tmp[jj] = feature[jj + point_id * nfeatures];
			...
			for (int i=0; i < nclusters; i++) {
				...
				for (int l = 0; l < nfeatures; l++){
					   ...
					   float feature_tmp = feature[l + point_id * nfeatures];
